...
+!open_bid(ArtifactID) : not i_am_winning(ArtifactID)
    <-  ?my_task(Task);
        ?my_reputation(MyRep);
        for( my_package(Place, RestaurantName, MyPrice)) {
            .print("Bidding in auction for ", Place, " to eat at ", RestaurantName, " with price: $", MyPrice);
            focus(ArtifactID);
            bid(MyPrice, MyRep, Place, RestaurantName);
        }.